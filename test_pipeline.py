...
with tf.Graph().as_default():
    with tf.Session() as sess:
        ...
        print('People in database: %3f' % len(class_names))
        ...
        while True:
            ...
            if nrof_faces > 0:
                ...
                for i in range(len(scaled)): 
                    ...
                    cv2.rectangle(frame,(bb[i][0],bb[i][1]),(bb[i][2],bb[i][3]),(0,0,255), thickness=2) #draw bounding box for the face
                    for j in range(len(best_class_indices)):
                        ...
                        if (best_class_probabilities[j] >=0.13): 
                            cv2.putText(frame, class_names[best_class_indices[j]], (bb[i][0], bb[i][1]-10), cv2.FONT_HERSHEY_COMPLEX_SMALL,1, (0,255,0), thickness=1, lineType=2)   
                        else:
                            cv2.putText(frame, 'Unknown', (bb[i][0], bb[i][1]), cv2.FONT_HERSHEY_COMPLEX_SMALL,
1, (255,0,0), thickness=1, lineType=2)   
                ...
                fps  = 1 / sec
                str = 'FPS: %2.3f' % fps
                cv2.putText(frame, str, (50, 50),cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 0, 0), thickness=1, lineType=2)
